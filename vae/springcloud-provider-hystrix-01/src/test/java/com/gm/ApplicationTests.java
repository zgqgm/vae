...
public class ApplicationTests {
    ...
    void generator() {
        // 数据库连接
        FastAutoGenerator.create("jdbc:mysql://localhost:3306/music?useSSL=false&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai",
                        "root",
                        "958824")
                // 全局配置
                .globalConfig(builder -> {
                    builder.author("猪猪侠") // 设置作者【会生成到注释中】
                ...
                .packageConfig(builder -> {
                    builder.parent("com.gm") // 设置父包名
                            //.moduleName("") // 设置父包模块名
                            .mapper("dao")   // 设置mapper接口的包名
                            .pathInfo(Collections.singletonMap(OutputFile.mapperXml, "src\\main\\resources\\com\\gm\\mapper")); // 设置mapperXml生成路径
                })
                .strategyConfig(builder -> {
                    builder
                            .enableSkipView() // 跳过视图【前后端分离就不需要使用视图】
                            .addInclude("personagetointroduce","tb_film","tb_image","tb_introduce","tb_literature","tb_music","tb_opus","tb_other","tb_personage","tb_prize","tb_sheet") // 设置需要生成的表名
